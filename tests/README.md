...
| `sabotage_invariants.py` | The live proof for the invariants (§0.7): writes a gitignored copy of the shipping workflow whose two invariant nodes are fed false evidence only when the inbound message carries `SABOTAGE-A` (1, 2, 3, 5, 4) or `SABOTAGE-B` (3b). Deploy, send the marked messages, observe, redeploy the real build | `python3 tests/sabotage_invariants.py`; procedure in the runbook section "The five invariants" |
