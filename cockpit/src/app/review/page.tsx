...
export default async function ReviewIndex() {
  ...
  return (
    <main style={{ maxWidth: 720, margin: '0 auto', padding: '48px 24px', fontSize: 16, lineHeight: 1.6 }}>
      ...
      <p style={{ color: '#555', marginTop: 0 }}>{REVIEW.intro}</p>
      <p style={{ color: '#555' }}>{REVIEW.pick}</p>
      <ul style={{ listStyle: 'none', padding: 0, marginTop: 24 }}>
        {clients.map((c) => (
          <li key={c.id} style={{ padding: '14px 0', borderTop: '1px solid #eee' }}>
            <Link href={`/review/${c.id}`} style={{ fontSize: 18, minHeight: 44, display: 'inline-block', lineHeight: '44px' }}>
              {c.name}
            </Link>
          </li>
        ))}
