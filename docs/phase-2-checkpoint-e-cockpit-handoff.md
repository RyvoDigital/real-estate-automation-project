...
**The luminance ladder is what makes a colour-free active state possible, rather than merely preferable.** It is a principle, not a finish, and it is the part that is easy to lose in a tidy-up:

> **Three surfaces, each one step lighter than the last: page, then the bar, then the active item.** A filled shape is not legible on its own. It is legible because it sits one rung above a bar that already sits one rung above the page. Remove the middle rung — flatten the bar into the page — and the active item stops reading as *selected* and starts reading as *a lighter rectangle*, at which point the only way to rescue it is colour, which is the thing this rule exists to avoid.

The rungs are **measured against the artefact they adapt**, not against a number someone picked: the reference bar's own steps read **1.13:1** and **1.32:1** when its screenshot is sampled pixel by pixel. Ours read 1.13:1 and 1.35:1. `npm run probe:contrast` asserts both, and a floor of 4.5:1 on every label in the bar.

Neither rung is near WCAG 1.4.11's 3:1, and neither should be. That 3:1 governs information *required* to identify a state; here the state is carried three ways at once — the fill step, the label going from `--ink-2` to `--ink`, and the icon. Asserting 3:1 on the fill alone would fail the design against a rule that does not apply to it, and would force a bar bright enough to compete with the breach card, which is the one thing this screen must never do.

What that measurement did surface is a real defect, and it was in the flush bar too: **idle tab labels at `--ink-3` read 3.34:1 against the bar — under 4.5:1.** They are `--ink-2` now, at 7.00:1. The desktop sidebar is a different surface and was left exactly as it was; whether `--ink-3` clears 4.5:1 against *its* gradient is an open question, not this one.
...
20. **Every screen is usable on a phone, measured in a browser at the widths real phones actually have.**
    ...
    Asserted per route as `document.documentElement.scrollWidth <= clientWidth` at **360px** (the common Android width), **390px** (iPhone 12–15) and **430px** (iPhone Pro Max), plus a smallest-rendered-type floor of 11px and a smallest-tap-target floor of 44px. `npm run probe:layout` drives the Chrome on the machine over the DevTools protocol; there is no browser dependency in `package.json`.
    ...
    **Desktop is the other half of the same file**, added after the mobile-first rewrite shipped a desktop regression: the sidebar rendered on the *right*. Nothing in the CSS was wrong. `Shell.tsx` renders `<main>` before `<nav>`, which is correct — content first for a screen reader, and on a phone the bar is `position: fixed` so source order cannot affect where it lands. On desktop the bar becomes a flex sibling and **source order silently becomes visual order**. Reading the stylesheet cannot catch that any more than it caught the phone bugs. At 1440px the probe asserts no sideways scroll per route, that the nav's right edge is left of the content's left edge, that a search field is ≤520px and a chat bubble ≤620px, and that filter chips wrap rather than hide in a scroller; at 1920px, that the app is centred rather than pinned to the left edge. Its control forces `order: 1` back onto `.tabs` and requires the sidebar assertion to fail — otherwise it would pass just as happily on a page with no sidebar at all.

    The wider lesson from that regression is worth keeping separate from the fix: the desktop block held **42 of the stylesheet's 735 declarations**. Nothing had leaked out of a breakpoint. Every value chosen for a 390px column — three stacked filter rows, a 1120px-wide "name or phone" field, bubbles at 88% of an 840px panel — simply stayed in force at 1440px because the enhancement never mentioned them. Mobile-first is not "one small media query at the end"; it is a base plus an enhancement that covers every rule whose *value* was a phone decision.
